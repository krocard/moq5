...
static moq_result_t loc_encode(const moq_alloc_t *al, const void *m,
                               moq_rcbuf_t **out)
{
    return moq_loc_encode(al, MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01,
...
static bool loc_reparse_equal(moq_bytes_t bytes, const void *m)
{
    ...
    if (moq_loc_parse(MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01, bytes, &got) != MOQ_OK) return false;
...
static moq_result_t loc_parse_only(moq_bytes_t bytes)
{
    ...
    return moq_loc_parse(MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01, bytes, &h);
}
