...
static int loc_soft(const moq_alloc_t *alloc, const uint8_t *buf, size_t len)
{
    ...
    if (moq_loc_parse(MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01, (moq_bytes_t){ buf, len }, &p1)
        != MOQ_OK)
    ...
    moq_rcbuf_t *re = NULL;
    moq_result_t er = moq_loc_encode(alloc, MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01, &p1, &re);
    MOQ_TEST_CHECK(er == MOQ_OK);
    if (er == MOQ_OK) {
        moq_bytes_t rb = re ? (moq_bytes_t){ moq_rcbuf_data(re), moq_rcbuf_len(re) }
                            : (moq_bytes_t){ NULL, 0 };
        moq_loc_headers_t p2;
        moq_result_t rc2 = moq_loc_parse(MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01, rb, &p2);
                      ...
                      const uint8_t *blob, size_t blen)
{
    ...
    if (kind == 0) {                         /* truncate at a random position */
        ...
        if (n > 0) {
            moq_loc_headers_t p1;
            moq_result_t rc = moq_loc_parse(MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01,
                        ...
                        uint64_t seed, int step, uint32_t mutate_permille)
{
    ...
    moq_result_t rc = moq_loc_encode(alloc, MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01, &model, &enc);
    ...
    moq_result_t prc = moq_loc_parse(MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01, bytes, &back);
    ...
    moq_result_t rc2 = moq_loc_encode(alloc, MOQ_VERSION_DRAFT_16, MOQ_LOC_PROFILE_01, &model, &enc2);
